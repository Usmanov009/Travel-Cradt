...
function formatPackageDetail(pkg, type) {
  ...
  const priceStr = pkg.price_currency === 'UZS' ? `${escMd(pkg.price?.toLocaleString())} so'm` : `$${pkg.price}`;
  return (
    ...
    try {
      ...
      if (rows.length === 0) {
        await ctx.reply(
          ...
          'Tur tanlash uchun /packages buyrug\'ini ishlating.',
          { parse_mode: 'MarkdownV2' }
        );
        return;
      }
      ...
      rows.forEach((b, i) => {
        ...
        const statusEmoji = b.status === 'accepted' ? '✅' : b.status === 'rejected' ? '❌' : '⏳';
        message += `${i + 1}\\. ${escMd(b.title)} — ${priceStr} ${statusEmoji}\n`;
        message += `   📅 ${b.created_at ? new Date(b.created_at).toLocaleDateString('uz-UZ') : '—'} | 👤 ${b.guests || 1} mehmon\n\n`;
      });
      
      message += 'Batafsil ma\'lumot uchun Web ilovani oching:';
      
      await ctx.reply(message, {
        parse_mode: 'MarkdownV2',
        ...Markup.inlineKeyboard([
          [Markup.button.webApp('📱 Web ilovada ochish', `${getWebAppUrl()}/dashboard`)],
          [Markup.button.callback('🏠 Bosh menyu', 'main_menu')],
        ]),
      });
    } catch (err) {
      console.error('Buyurtmalarni olish xatosi:', err.message);
...
async function showPackageDetail(ctx, type, localId) {
  ...
  await ctx.reply(text, {
    parse_mode: 'MarkdownV2',
    ...Markup.inlineKeyboard([
      [Markup.button.webApp('🎫 Band qilish', `${url}/package/${type}/${pkg.id || pkg.local_id || localId}`)],
