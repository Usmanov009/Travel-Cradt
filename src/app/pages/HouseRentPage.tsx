...
import { SAMPLE_HOUSE_RENTALS } from "../data/sampleListings";

export function HouseRentPage() {
  const { t } = useTranslation();
  const { companies } = useCompanies();
  const [selectedCompanyId, setSelectedCompanyId] = useState<number | null>(null);

  const listings = SAMPLE_HOUSE_RENTALS.map((item, i) => {
      ...
      <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-12 md:py-20">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 mb-4">
            <Home className="w-6 h-6 md:w-8 md:h-8 shrink-0" />
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold">{t("houseRent.title")}</h1>
          </div>
          <p className="text-base sm:text-xl text-blue-100 max-w-2xl">{t("houseRent.desc")}</p>
        </div>
      </div>
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="text-sm font-semibold text-blue-600 mb-3">
                  ...
                  <span className="text-slate-400 font-normal">/ {t("houseRent.perNight")}</span>
                </p>
                {item.companyName ? (
                  <button
